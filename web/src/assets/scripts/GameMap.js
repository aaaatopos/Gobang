...
export class GameMap extends AcGameObject {
    constructor(ctx, parent, store) {
        ...
        this.players = [
            new Player({id: 0, colorIn: "#636766", colorOut: "#0D0D0D"}, this),
            new Player({id: 1, colorIn: "#F9F9F9", colorOut: "#AAAAAA"}, this),
        ];
        this.steps = 0;
    }
    ...
    add_listening_events() {
        if(this.store.state.record.is_record) {  // 如果是录像
            ...
            const interval_id = setInterval(() => {
                if(k < totalStepNum && k % 2 == 0) {
                    player0.set_next_position(parseInt(a_steps_x[aIdx]), parseInt(a_steps_y[aIdx]));
                    this.store.commit("updateCurrentPlayer", "A");
                    aIdx ++;
                } else if(k < totalStepNum && k % 2 != 0) {
                    player1.set_next_position(parseInt(b_steps_x[bIdx]), parseInt(b_steps_y[bIdx]));
    ...
    render() {
        ...
        for(let star of this.stars) {
            ...
            this.ctx.beginPath();
            this.ctx.arc((x + 0.5) * this.L, (y + 0.5) * this.L, this.L / 4, 0, 2 * Math.PI);
            this.ctx.fill();
            this.ctx.closePath();
        }
    }
